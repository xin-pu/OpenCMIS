...
namespace OpenCMIS.Transport.Abstractions.Tests
{
    public sealed class I2cConnectionProfileTests
    {
        ...
        {
            Assert.Throws<ArgumentException>(() => new SerialI2cConnectionProfile(
                                                     "linktel",
                                                     "",
                                                     115200,
                                                     ModuleAddress));
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => new HmMultiChannelConnectionProfile(
                                                               "hm-multichannel",
                                                               "COM8",
                                                               1500000,
                                                               channel,
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => new I2cTransferCapabilities(0,  32));
            Assert.Throws<ArgumentOutOfRangeException>(() => new I2cTransferCapabilities(32, 0));
        }

        [Fact]
        public void Retry_options_reject_non_positive_attempt_count()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => new I2cRetryOptions(0, TimeSpan.Zero));
        }

        [Fact]
        public void Retry_options_reject_negative_delay()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => new I2cRetryOptions(1, TimeSpan.FromMilliseconds(-1)));
        }
    }
}
