...
namespace OpenCMIS.Transport.I2C.Cypress.Tests
{
    public sealed class CypressI2cAdapterTests
    {
        ...
        public async Task Fic2Usb_write_maps_port_speed_address_and_register_prefix()
        {
            ...
            await using var adapter = new Fic2UsbI2cAdapter(api, 1, 100);
        ...
        public async Task Read_sets_register_pointer_then_copies_exact_result()
        {
            ...
            await using var adapter = new Fic2UsbI2cAdapter(api, 0, 400);
            await adapter.OpenAsync();
        ...
        public async Task Eui3_false_transfer_is_converted_to_i2c_transfer_failed()
        {
            ...
            await using var adapter = new Eui3I2cAdapter(api, 0, 400);
            await adapter.OpenAsync();

            var error = await Assert.ThrowsAsync<CmisException>(() => adapter.WriteAsync(
                                                                                      Address,
                                                                                      new (0x10),
                                                                                      new byte[] {0x01})
                                                                             .AsTask());
        ...
        {
            var             api     = new MockCypressDeviceApi();
            await using var adapter = new Eui3I2cAdapter(api, 0, 90);
            await adapter.OpenAsync();
        ...
        {
            var             api     = new MockCypressDeviceApi();
            await using var adapter = new Fic2UsbI2cAdapter(api, 0, 100);
            await adapter.OpenAsync();
            using var cancellation = new CancellationTokenSource();
            cancellation.Cancel();

            await Assert.ThrowsAnyAsync<OperationCanceledException>(() => adapter.ReadAsync(
                                                                                          Address,
                                                                                          new (0),
                                                                                          new byte[1],
                                                                                          cancellation.Token)
        ...
        public async Task Short_low_level_read_is_rejected_as_transfer_failure()
        {
            ...
            var error = await Assert.ThrowsAsync<CmisException>(() => adapter.ReadAsync(
                                                                                      Address,
                                                                                      new (0),
                                                                                      new byte[2])
                                                                             .AsTask());
        ...
        {
            var api     = new MockCypressDeviceApi();
            var adapter = new Fic2UsbI2cAdapter(api, 0, 100);
            await adapter.OpenAsync();
