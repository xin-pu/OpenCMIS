...
namespace OpenCMIS.Transport.I2C.Serial.Tests
{
    public sealed class SerialI2cAdapterTests
    {
        ...
        public async Task Linktel_read_honors_explicit_device_address()
        {
            ...
            await adapter.ReadAsync(new (0x51), Offset, destination);
        ...
        public async Task Linktel_read_segments_at_255_bytes()
        {
            ...
            await adapter.ReadAsync(Address, new (0x00), destination);
        ...
        public async Task Adapter_does_not_retry_invalid_response()
        {
            ...
            var error = await Assert.ThrowsAsync<CmisException>(() => adapter.ReadAsync(Address, Offset, new byte[1]).AsTask());
        ...
        public async Task Hm_multichannel_uses_channel_specific_commands(byte channel,
                                                                         byte readCommand,
                                                                         byte writeCommand)
        {
            var sessions = new ScriptedSerialSessionFactory(
        ...
        public async Task Hm_write_rejects_failed_acknowledgement()
        {
            ...
            var error = await Assert.ThrowsAsync<CmisException>(() => adapter.WriteAsync(Address, Offset, new byte[] {0x12}).AsTask());

            Assert.Equal(CmisErrorCode.I2cInvalidResponse, error.ErrorCode);
        }

        private static LinktelSerialI2cAdapter CreateLinktel(ScriptedSerialSessionFactory sessions,
