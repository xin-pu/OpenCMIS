...
namespace OpenCMIS.App.Core.Tests
{
    public sealed class CmisDeviceCompatibilityTests
    {
        ...
        {
            var msa = new StubMsaMemoryAccessor()
                     .Returns(CmisConstants.ThresholdPage, CmisConstants.RegTempHighAlarmMSB, [0x00, 0x80])
                     .Returns(CmisConstants.ThresholdPage, CmisConstants.RegTempLowAlarmMSB,  [0x00, 0x00])
                     .Returns(CmisConstants.ThresholdPage, CmisConstants.RegTempHighWarnMSB,  [0x00, 0x80])
                     .Returns(CmisConstants.ThresholdPage, CmisConstants.RegTempLowWarnMSB,   [0x00, 0x00])
                     .Returns(CmisConstants.ThresholdPage, CmisConstants.RegVccHighAlarmMSB,  [0xFF, 0xFF])
                     .Returns(CmisConstants.ThresholdPage, CmisConstants.RegVccLowAlarmMSB,   [0x00, 0x00])
                     .Returns(CmisConstants.ThresholdPage, CmisConstants.RegVccHighWarnMSB,   [0xFF, 0xFF])
        ...
        private sealed class StubMsaMemoryAccessor : IMsaMemoryAccessor
        {
            ...
            public StubMsaMemoryAccessor Returns(byte   page,
                                                 byte   offset,
                                                 byte[] data)
            {
        ...
        private sealed class StubHciMemoryAccessor : IHciMemoryAccessor
        {
            public ValueTask<byte[]> ReadAsync(I2cDeviceAddress  device,
                                               ...
                                               CancellationToken cancellationToken = default)
            {
                return ValueTask.FromResult(new byte[length]);
            }
