...
namespace OpenCMIS.Cypress
{
    ...
    public class CyUSBDevice : USBDevice
    {
        ...
        internal override bool Open(byte dev)
        {
            ...
            if ((_bcdUSB & CyConst.bcdUSBJJMask) == CyConst.USB30MajorVer)
            {
                ...
                try
                {
                    USBBos = new (_hDevice, _usb30BosDescriptors);
                }
                catch (Exception exc)
                {
            ...
            ControlEndPt = new (_hDevice, _maxPacketSize);
            ...
            for (var i = 0; i < _configs; i++)
            {
                ...
                {
                    if ((_bcdUSB & CyConst.bcdUSBJJMask) == CyConst.USB20MajorVer)
                        USBCfgs[i] = new (_hDevice, _usbConfigDescriptors[i], ControlEndPt);
                    else
                    {
                        byte usb30Dummy = 1; // it's dummy variable to call the usb3.0 specific constructor
                        USBCfgs[i] = new (_hDevice, _usbConfigDescriptors[i], ControlEndPt, usb30Dummy);
                    }
                }
                catch (Exception exc)
                {
        ...
        private readonly byte[][] _usbConfigDescriptors = new byte[2][];

        private readonly byte[] _usb30BosDescriptors = new byte[CyConst.MaxDescriptorBufferLength];

        private byte _interfaces;
        private byte _altInterfaces;
        private byte _configs;

        private byte _cfgNum;
        private byte _intfcNum;   // The current selected interface's bInterfaceNumber
        private byte _intfcIndex; // The entry in the Config's interfaces table matching to IntfcNum and AltSetting

        private readonly int[] _bytesXfered = new int[1];
        private          uint  _usbdStatus;
        private          uint  _ntStatus;

        private unsafe bool IoControl(uint cmd, byte[] buf, int len)
        {
            ...
            fixed (byte * buf = buffer)
            {
                ...
                if (bRetVal)
                {
                    ...
                    if ((_bcdUSB & CyConst.bcdUSBJJMask) == CyConst.USB20MajorVer)
                        ...
                        _maxPacketSize = 1 << descriptor->bMaxPacketSize0;
        ...
        private unsafe bool GetBosDescriptor()
        {
            ...
            fixed (byte * buf = buffer)
            {
                ...
                transfer->SetupPacket.bmRequest = CyConst.TGT_DEVICE | CyConst.REQ_STD | CyConst.DIR_FROM_DEVICE;
                transfer->SetupPacket.bRequest  = CyConst.USB_REQUEST_GET_DESCRIPTOR;
                transfer->SetupPacket.wValue    = CyConst.USB_BOS_DESCRIPTOR_TYPE << 8;
        ...
        private unsafe void SetStringDescrLanguage()
        {
 ...
 Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\Class\\{36FC9E60-C465-11CF-8056-444553540000}\\");
        ...
        private void SetAltIntfcParams(byte alt)
        {
            // Find match of IntfcNum and alt in table of interfaces
            if (USBCfgs[_cfgNum] != null)
            {
