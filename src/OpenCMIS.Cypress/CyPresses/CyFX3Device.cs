...
namespace OpenCMIS.Cypress
{
    public class CyFX3Device : CyUSBDevice
    {
        ...
        private FileStream m_script_file_name;

        public bool m_bRecording;
        public byte m_ConfigNum;
        public byte m_IntfcNum;
        public byte m_AltIntfc;

        internal CyFX3Device()
                : this(CyConst.CyGuid) { }
        ...
        public string GetFwErrorString(FX3_FWDWNLOAD_ERROR_CODE eFwErrorCode)
        {
            switch (eFwErrorCode)
            {
                ...
                case FX3_FWDWNLOAD_ERROR_CODE.I2CEEPROM_UNKNOWN_I2C_SIZE:
                    return "Unknown I2CE2PROM size, Unknown value parsed from 2nd Bytes of IMG file";
        ...
        internal FX3_FWDWNLOAD_ERROR_CODE DownloadFwToRam(ref byte[] buf, ref uint buflen)
        {
        ...
        internal FX3_FWDWNLOAD_ERROR_CODE DownloadUserIMGtoI2CE2PROM(ref byte[] buf, ref uint buflen)
        {
            ...
            for (uint i = 0; i < NoOfStage; i++)
            {
                ...
                ControlEndPt.Index +=
                        (ushort) StageSize; //Starting address  withing I2C chunk size(for I2C size :32Kbyte the Index will go like 0,4,8,12,16 etc..)
