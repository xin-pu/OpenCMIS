...
public abstract class SerialI2cAdapterBase : II2cRegisterBus
{
    ...
    protected SerialI2cAdapterBase(
        ...
        TimeProvider timeProvider)
    {
        ...
        PortName = portName;
        BaudRate = baudRate;
        _retry = new SerialTransferRetry(retryOptions, timeProvider);
    }
    ...
    protected abstract byte[] EncodeRead(
        I2cDeviceAddress device,
        RegisterOffset offset,
        int length);

    protected abstract byte[] EncodeWrite(
        I2cDeviceAddress device,
        RegisterOffset offset,
        ReadOnlySpan<byte> data);

    protected abstract byte[] ParseRead(
    ...
    private async ValueTask<byte[]> ReadChunkAsync(
        I2cDeviceAddress device,
        RegisterOffset offset,
        int length,
