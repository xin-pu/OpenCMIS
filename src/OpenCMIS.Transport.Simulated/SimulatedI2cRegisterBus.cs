...
public sealed class SimulatedI2cRegisterBus : II2cRegisterBus
{
    ...
    internal SimulatedI2cRegisterBus(
        string profile,
        int seed = 42,
    ...
    internal void SetNoiseEnabled(bool enabled) => _noiseEnabled = enabled;
    ...
    private byte ApplyNoise(byte original, byte bank, byte page, byte address)
    {
        ...
        var result = (int)original + jitter;
    ...
    private void PopulateIdentity(string profile)
    {
        ...
        SetByte(0, 0x00, (byte)(CmisConstants.RegInterruptFlags + 1), 0x00);
        ...
        SetByte(0, 0x00, (byte)(CmisConstants.RegModuleFlags + 1), maxRate);
    ...
    private void SetAscii(
        byte bank, byte page, byte startAddress, int maxLength, string text)
    {
