...
namespace OpenCMIS.Module.Core.Hci
{
    ...
    public static class HciCommandCodec
    {
        ...
        public static byte[] EncodeRead(HciTableId     table,
                                        RegisterOffset offset,
                                        int            length)
        {
            ValidateLength(length);
            return [ReadCommand, 0x00, 0x00, table.Value, offset.Value, ControlFlag, (byte) length];
        }

        public static byte[] EncodeWrite(HciTableId         table,
                                         RegisterOffset     offset,
        ...
        public static byte[] ExtractReadPayload(ReadOnlySpan<byte> response,
                                                int                requestedLength)
        {
