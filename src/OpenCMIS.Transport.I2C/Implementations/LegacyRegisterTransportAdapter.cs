...
public abstract class LegacyRegisterTransportAdapter : IRegisterTransport
{
    ...
    protected LegacyRegisterTransportAdapter(
    ...
    public async Task<byte[]> ReadRegisterBlockAsync(
        ...
        int length)
    {
        ...
        await _inner.ReadAsync(
                _device,
                new RegisterOffset(registerAddress),
                data)
            .ConfigureAwait(false);
        return data;
    }

    public Task WriteRegisterBlockAsync(
        byte registerAddress,
        byte[] data)
    {
        ThrowIfDisposed();
